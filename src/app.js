...
function saveData() {
  try {
    localStorage.setItem('cg_v2_' + state.currentYear, JSON.stringify(state));
    markSaved();
...
function markSaved() {
  ...
  el.textContent = `💾 Salvato alle ${hh}:${mm}`;
  el.classList.remove('save-indicator-error');
  el.classList.add('save-indicator-flash');
  clearTimeout(window.__saveIndicatorTimer);
  window.__saveIndicatorTimer = setTimeout(() => el.classList.remove('save-indicator-flash'), 900);
}
function markSaveError() {
  const el = document.getElementById('save-indicator');
  if (!el) return;
  el.textContent = '⚠️ Salvataggio non riuscito — scarica un backup ora';
...
function loadData() {
  ...
  updateLockUI();
  const el = document.getElementById('save-indicator');
  if (el) el.textContent = restored ? '💾 Dati della sessione precedente ripristinati' : '💾 Salvataggio automatico attivo';
}
  ...
  function makeRow(section, idx, row, color) {
    ...
    return `<tr class="${color}">
      <td style="display:flex;align-items:center;gap:4px">
        ${typeBtn}
        <input class="fp-name-inp" type="text" value="${row.name.replace(/"/g,'&quot;')}" placeholder="Nome voce..."
    ...
    <table class="data-table">
      ...
      <tbody>
        ...
        ${!state.locked?`<tr><td colspan="15" style="padding:4px">
          <button class="btn btn-outline btn-sm" onclick="addFPRow('income')">+ Aggiungi voce ricavo</button>
        </td></tr>`:''}
        ...
        ${!state.locked?`<tr><td colspan="15" style="padding:4px">
          <button class="btn btn-outline btn-sm" onclick="addFPRow('vcosts')">+ Aggiungi costo variabile</button>
        </td></tr>`:''}
        ...
        ${!state.locked?`<tr><td colspan="15" style="padding:4px">
          <button class="btn btn-outline btn-sm" onclick="addFPRow('fcosts')">+ Aggiungi costo fisso</button>
        </td></tr>`:''}
...
function addFPRow(section) {
  const costType = section === 'vcosts' ? 'V' : section === 'fcosts' ? 'F' : undefined;
  D().fp[section].push({name: '', monthly: new Array(12).fill(0), costType});
  saveData(); renderModule();
  // Porta subito il focus sul nome della nuova riga: niente popup, si scrive direttamente in tabella
  setTimeout(() => {
    const inputs = document.querySelectorAll('.fp-name-inp');
...
function showImportModal() {
  ...
  if (tab === 'movimenti') {
    ...
    ${importState.rows.length > 0 ? renderCategorizePreview() : ''}
...
// ═══════════════════════════════════════════════════════════
// IMPUTAZIONE AUTOMATICA A CENTRO DI COSTO/RICAVO
// Usata da import Movimenti Bancari (Excel/CSV) e da import PDF.
// Ogni riga viene proposta con: Entrata/Uscita, Voce di Piano Finanziario
// (centro di costo/ricavo) e Tipologia Cashflow — tutte modificabili
// prima di confermare l'importazione nel Registro Movimenti.
// ═══════════════════════════════════════════════════════════
const IMPORT_KEYWORDS = {
  'affitto': ['affitto','locazione','canone locazione'],
  'utenze': ['enel','eni gas','a2a','hera','acea','iren','edison','sorgenia','gas natural','telecom','tim spa','vodafone','wind tre','fastweb','acquedotto'],
  'commercialista': ['commercialista','dottore commercialista','studio associato','parcella professionale'],
  'assicura': ['assicura','unipol','generali','allianz','axa','reale mutua','vittoria assicurazioni'],
  'personale': ['stipendio','stipendi','busta paga','tfr','f24 inps','inps '],
  'tributi': ['f24','agenzia entrate','erario','iva ','tributi'],
  'fornitor': ['fornitore','pagamento fattura','bonifico a favore'],
  'carburante': ['esso','eni station','ip carburanti',' q8 ','tamoil','distributore carburante'],
  'leasing': ['leasing','noleggio lungo termine','locazione finanziaria'],
  'banca': ['commissioni','spese tenuta conto','canone conto','imposta di bollo','competenze bancarie']
};
function guessImportCategory(descrizione, tipo) {
  const desc = (' ' + (descrizione || '').toLowerCase() + ' ');
  const cfg = SECTORS[state.currentSector];
  const fp = D().fp;
  let candidatesFP, candidatesCF;
  if (tipo === 'entrate') {
    candidatesFP = [...new Set([...fp.income.map(r => r.name).filter(Boolean), ...cfg.income])];
    candidatesCF = cfg.cashflowIn;
  } else {
    candidatesFP = [...new Set([...fp.vcosts.map(r => r.name).filter(Boolean), ...fp.fcosts.map(r => r.name).filter(Boolean), ...cfg.vcosts, ...cfg.fcosts])];
    candidatesCF = cfg.cashflowOut;
  }
  function scoreMatch(name) {
    const n = (name || '').toLowerCase();
    if (!n) return 0;
    if (desc.includes(n)) return 100;
    const words = n.split(/[^a-zàèéìòù0-9]+/).filter(w => w.length > 3);
    let score = 0;
    words.forEach(w => { if (desc.includes(w)) score += 10; });
    return score;
  }
  let best = null, bestScore = 0;
  candidatesFP.forEach(name => {
    const s = scoreMatch(name);
    if (s > bestScore) { bestScore = s; best = name; }
  });
  if (bestScore < 10) {
    // Il testo del movimento contiene una parola-spia (es. "enel" → categoria "utenze");
    // cerchiamo poi tra le voci del cliente quella il cui nome contiene la categoria (es. "Utenze sede").
    for (const [cat, kws] of Object.entries(IMPORT_KEYWORDS)) {
      if (kws.some(kw => desc.includes(kw))) {
        const match = candidatesFP.find(name => (name || '').toLowerCase().includes(cat));
        if (match) { best = match; bestScore = 40; break; }
      }
    }
  }
  return {
    tipologiaFP: best || candidatesFP[0] || '',
    tipologiaCF: candidatesCF[0] || '',
    matched: bestScore >= 10
  };
}
function enrichImportRows(rows) {
  rows.forEach(r => {
    if (!r.tipo) r.tipo = (parseFloat(r.importo) || 0) >= 0 ? 'entrate' : 'uscite';
    const g = guessImportCategory(r.descrizione, r.tipo);
    r.tipologiaFP = g.tipologiaFP;
    r.tipologiaCF = g.tipologiaCF;
    r.matched = g.matched;
  });
  return rows;
}
function renderCategorizePreview() {
  const rows = importState.rows;
  if (!rows.length) return '';
  const cfg = SECTORS[state.currentSector];
  const fp = D().fp;
  const incomeCat = [...new Set([...fp.income.map(r => r.name).filter(Boolean), ...cfg.income])];
  const costCat = [...new Set([...fp.vcosts.map(r => r.name).filter(Boolean), ...fp.fcosts.map(r => r.name).filter(Boolean), ...cfg.vcosts, ...cfg.fcosts])];
  const nMatched = rows.filter(r => r.matched).length;
  const shown = rows.slice(0, 200);
  const rowsHtml = shown.map((r, i) => {
    const isEnt = r.tipo === 'entrate';
    const fpOpts = isEnt ? incomeCat : costCat;
    const cfOpts = isEnt ? cfg.cashflowIn : cfg.cashflowOut;
    return `<tr class="${r.matched ? '' : 'import-row-unmatched'}">
      <td style="white-space:nowrap">${r.data || ''}</td>
      <td style="min-width:160px">${String(r.descrizione || '').replace(/</g,'&lt;')}</td>
      <td style="text-align:right;white-space:nowrap">${fc(Math.abs(parseFloat(r.importo) || 0))}</td>
      <td><select onchange="updateImportRowType(${i},this.value)" style="width:auto">
        <option value="entrate" ${isEnt ? 'selected' : ''}>Entrata</option>
        <option value="uscite" ${!isEnt ? 'selected' : ''}>Uscita</option>
      </select></td>
      <td><select onchange="updateImportRow(${i},'tipologiaFP',this.value)" style="min-width:170px">
        ${fpOpts.map(c => `<option ${r.tipologiaFP===c?'selected':''}>${c.replace(/"/g,'&quot;')}</option>`).join('')}
      </select></td>
      <td><select onchange="updateImportRow(${i},'tipologiaCF',this.value)" style="min-width:150px">
        ${cfOpts.map(c => `<option ${r.tipologiaCF===c?'selected':''}>${c.replace(/"/g,'&quot;')}</option>`).join('')}
      </select></td>
    </tr>`;
  }).join('');
  const allOk = nMatched === rows.length;
  return `
  <div class="import-summary ${allOk ? 'import-summary-ok' : 'import-summary-warn'}">
    ${allOk ? '✅' : '⚠️'} <b>${nMatched}/${rows.length}</b> righe imputate automaticamente al centro di costo/ricavo giusto${!allOk ? ' — controlla le righe evidenziate in giallo' : ''}.
    Puoi correggere ogni riga dai menu a tendina prima di importare.
  </div>
  <div class="import-preview">
    <table>
      <thead><tr><th>Data</th><th>Descrizione</th><th style="text-align:right">Importo</th><th>Tipo</th><th>Centro di Costo/Ricavo</th><th>Cashflow</th></tr></thead>
      <tbody>${rowsHtml}</tbody>
    </table>
  </div>
  <div style="font-size:11px;color:var(--muted);margin-top:4px">${rows.length} righe trovate${rows.length>200?' (mostrando prime 200 — le altre verranno importate con la stessa logica automatica)':''}</div>`;
}
function updateImportRow(i, field, value) {
  importState.rows[i][field] = value;
}
function updateImportRowType(i, value) {
  const r = importState.rows[i];
  r.tipo = value;
  const g = guessImportCategory(r.descrizione, value);
  r.tipologiaFP = g.tipologiaFP;
  r.tipologiaCF = g.tipologiaCF;
  r.matched = g.matched;
  showImportModal();
  ...
  reader.onload = e => {
    try {
      ...
      if (importState.tab === 'piano') {
        ...
        enrichImportRows(importState.rows);
      }
      showImportModal();
...
function confirmImport() {
  const cfg = SECTORS[state.currentSector];
  let nEnt = 0, nUsc = 0, nDaVerificare = 0;
  importState.rows.forEach(r => {
    const imp = parseFloat(r.importo) || 0;
    const tipo = r.tipo || (imp >= 0 ? 'entrate' : 'uscite');
    const row = {
      data: r.data || '',
      importoLordo: Math.abs(imp),
      descrizione: r.descrizione || '',
      tipologiaCF: r.tipologiaCF || (tipo === 'entrate' ? (cfg.cashflowIn[0] || '') : (cfg.cashflowOut[0] || '')),
      tipologiaFP: r.tipologiaFP || (tipo === 'entrate' ? (cfg.income[0] || '') : (cfg.fcosts[0] || cfg.vcosts[0] || '')),
      iva: 22
    };
    D().registro[tipo].push(row);
    if (tipo === 'entrate') nEnt++; else nUsc++;
    if (!r.matched) nDaVerificare++;
  });
  saveData();
  closeModal();
  ...
  renderModule();
  const nota = nDaVerificare > 0 ? `\n⚠️ ${nDaVerificare} righe non erano state riconosciute con certezza: verificane la categoria nel Registro.` : '\n✅ Tutte le righe erano state riconosciute automaticamente.';
  alert(`✅ Importati ${nEnt} entrate e ${nUsc} uscite nel Registro Movimenti, già imputati ai centri di costo/ricavo.${nota}`);
}
